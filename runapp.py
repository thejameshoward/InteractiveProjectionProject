...
from app.components.axes_selector import get_axes_selection_div, failure_div

from app.styles import *
from app.styles import button_style, SCATTERPLOT_TEXT_FONT_COLOR, upload_button_style
#
...
def update_output(list_of_contents, n_clicks, list_of_names, list_of_dates):
    ...
    if trigger_id == 'example-button':
        with open('./data/EXAMPLE_UMAP.csv', 'rb') as infile:
            data = infile.read()
        content_bytes = base64.b64encode(data)
        content_string = content_bytes.decode("utf-8")

        mime = magic.Magic(mime=True)
        mime_type = mime.from_file('./data/EXAMPLE_UMAP.csv')
